...
- **Прод-окружение (АКТУАЛЬНО на 2026-05-23 — переехал на Docker):** VPS `176.108.252.98` (2 vCPU / 4 ГБ + swap 4G). **App и БД запускаются через `docker compose` из `/home/nikit/planwise`** (сервисы `db` + одноразовый `migrate` + `app`). `app` слушает `127.0.0.1:3000`, наружу только **nginx на `:80`** (`proxy_pass localhost:3000`, конфиг не менялся при переезде). Образ собирается из исходников **в контейнере** (`docker compose up --build`), серты «Минцифры» вшиты (`NODE_OPTIONS=--use-system-ca`) → TLS к GigaChat без обхода. Секреты приложения — в gitignored **`.env`** (туда сведены `.env.production` + `POSTGRES_PASSWORD`; `AUTH_URL=http://176.108.252.98`). `restart: unless-stopped` на `app`+`db` → переживают ребут (systemd больше НЕ нужен для автозапуска). Фаервол **ufw** (только `22`, `80`).
