...
export function buildRovSkeletonMessages(
  ...
  // Безусловный блок: ценности Указа 809 применимы к каждому РоВ-сценарию
  // (в отличие от personalResultsBlock, который зависит от наличия direction).
  // Лимиты ниже выровнены со схемой scenarioContentSchema (secondaryValues.max(3), valueFormulations.max(8)).
  const valuesCatalogBlock = [
    ...
    'Опционально 0–8 живых формулировок темы в "valueFormulations": каждая {text, basedOn},',
...
export function buildRovBlockMessages(
  ...
  const isGreetingBlock = stage.kind === 'engage' && /приветств/iu.test(brief.focus)

  const videoPolicy: string[] = isVideoBlock
    ? [
        '',
        'ВИДЕОВХОД:',
  ...
  const greetingPolicy: string[] = isGreetingBlock
    ? [
        '',
        'ПРИВЕТСТВИЕ:',
