...
export function buildRovSkeletonMessages(
  ...
  const system = [
    ...
    '- Первый блок мотивационной части — обсуждение видеоматериала. Видео учитель подберёт сам; в задании пиши «просмотр и обсуждение короткого видеоролика по теме» и приводи 2-3 ключевых вопроса для обсуждения. НЕ выдумывай ссылку и конкретное название ролика.',
...
// Keep the old name as alias for backward compat (prompt.ts barrel exports it)
