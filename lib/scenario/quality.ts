...
export function checkScenario(
  ...
  // Физкультминутка — СП 2.4.3648-20 п. 2.10.2 / 2.10.3 предусматривает введение
  // физкультминуток во время занятий для профилактики нарушений осанки и при работе
  // с ЭСО. Триггер: занятие 40+ мин в 1-9 классах без явного упоминания двигательной
  // паузы. СОО и короткие занятия не задеваются — на них требование мягче.
  if (
    opts?.grade !== undefined &&
    opts.grade >= 1 &&
    opts.grade <= 9 &&
    (opts.durationMin ?? 0) >= 40
  ) {
    ...
    if (!/физкульт|двигат|встань|разминк/.test(allText)) {
      warnings.push(
        'На занятии 40+ мин в 1-9 классах рекомендуется физкультминутка (СП 2.4.3648-20 п. 2.10)',
      )
    }
  }
