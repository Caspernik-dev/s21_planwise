...
function loadConfig(cwd, options = {}) {
  ...
  if (ws) {
    ...
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(rootParsed, 'branching_strategy')) {
        ...
        try {
          platformWriteSync(rootConfigPath, JSON.stringify(rootParsed, null, 2))
        } catch {}
      }
    } catch {
  ...
  try {
    ...
    if ('depth' in fileData && !('granularity' in fileData)) {
      ...
      try {
        platformWriteSync(configPath, JSON.stringify(fileData, null, 2))
      } catch {
        /* intentionally empty */
      }
    }
  ...
  try {
    ...
    if (versionOverride) {
      ...
      if (!sectionMatch) {
        ...
        const hasVersionedMilestones = /^#{1,3}\s+.*v\d+\.\d+/im.test(roadmapContent)
