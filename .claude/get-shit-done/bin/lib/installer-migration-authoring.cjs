...
function recordSource(record, fallback) {
  return (
    fallback ||
    (record && typeof record.id === 'string' && record.id.trim() ? record.id : '<unknown>')
  )
}
...
function requireActionEvidence(action, field, migration) {
  if (typeof action[field] !== 'string' || action[field].trim() === '') {
    throw new Error(
      `migration action ${action.type} must include ${field}: ${actionSource(migration, action)}`,
    )
  }
}
...
function validateInstallerMigrationActions(actions, migration) {
  ...
  for (const action of actions) {
    ...
    if (typeof action.relPath !== 'string' || action.relPath.trim() === '') {
      throw new Error(
        `migration action ${action.type} must include a non-empty relPath: ${migration.id}`,
      )
    }
    ...
    if (
      ...
      (typeof migration.runtimeContract !== 'string' || migration.runtimeContract.trim() === '')
    ) {
      throw new Error(
        `migration action rewrite-json requires migration runtimeContract: ${actionSource(migration, action)}`,
      )
    }
  }
