...
function validateShipPrBodySections(value) {
  ...
  value.forEach((section, index) => {
    ...
    if (typeof section.source === 'string' && section.source.trim() !== '') {
      const selectors = section.source
        .split('||')
...
function cmdConfigSet(cwd, keyPath, value, raw) {
  ...
  if (
    keyPath === 'workflow.human_verify_mode' &&
    !VALID_HUMAN_VERIFY_MODES.includes(String(parsedValue))
  ) {
    error(
      `Invalid workflow.human_verify_mode '${value}'. Valid values: ${VALID_HUMAN_VERIFY_MODES.join(', ')}`,
    )
  }
  ...
  if (
    keyPath === 'statusline.context_position' &&
    !VALID_CONTEXT_POSITIONS.includes(String(parsedValue))
  ) {
    error(
      `Invalid statusline.context_position '${value}'. Valid values: ${VALID_CONTEXT_POSITIONS.join(', ')}`,
    )
  }
  ...
  if (
    keyPath === 'code_quality.fallow.scope' &&
    !VALID_FALLOW_SCOPES.includes(String(parsedValue))
  ) {
    error(
      `Invalid code_quality.fallow.scope '${value}'. Valid values: ${VALID_FALLOW_SCOPES.join(', ')}`,
    )
  }
  const VALID_FALLOW_PROFILES = ['minimal', 'standard', 'strict']
  if (
    keyPath === 'code_quality.fallow.profile' &&
    !VALID_FALLOW_PROFILES.includes(String(parsedValue))
  ) {
    error(
      `Invalid code_quality.fallow.profile '${value}'. Valid values: ${VALID_FALLOW_PROFILES.join(', ')}`,
    )
  }
