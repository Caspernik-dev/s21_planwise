...
const {
  ...
  platformReadSync,
  platformEnsureDir,
} = require('./shell-command-projection.cjs')
...
function cmdStateSnapshot(cwd, raw) {
  ...
  const currentPhaseName =
    fmScalar('current_phase_name') ?? stateExtractField(body, 'Current Phase Name')
  ...
  const totalPlansRaw =
    fmScalar('total_plans_in_phase') ?? stateExtractField(body, 'Total Plans in Phase')
  ...
  const lastActivityDesc =
    fmScalar('last_activity_desc') ?? stateExtractField(body, 'Last Activity Description')
...
function updatePerformanceMetricsSection(content, cwd, phaseNum, planCount, summaryCount) {
  ...
  if (byPhaseMatch) {
    ...
    content = content.replace(
      byPhaseTablePattern,
      (_match, tableHeader) => `${tableHeader}${tableBody}\n`,
    )
  }
...
function cmdStateSync(cwd, options, raw) {
  ...
  const percent = (() => {
    const p = computeProgressPercent(
      totalDiskSummaries,
      totalDiskPlans,
      diskCompletedPhases,
      syncTotalPhases,
