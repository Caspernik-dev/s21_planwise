...
- **Global scope**: `/home/nikit/planwise/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/nikit/planwise/.claude/notes/`)
...
2. Glob `/home/nikit/planwise/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/nikit/planwise/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/nikit/planwise/.claude/notes/` — works in any directory
