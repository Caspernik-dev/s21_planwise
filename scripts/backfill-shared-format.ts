/**
 * Backfill shared_scenarios.anonymized_content to the new format.
 *
 * Зеркалит scripts/backfill-scenario-format.ts, но для расшаренных сценариев:
 *  - в shared_scenarios нет user_id (изоляция не нужна)
 *  - нет таблицы версий — обновление в одной транзакции с UPDATE
 *  - 15 строк по проду — копится быстро
 *
 * Поля добавляются из каталогов (ФГОС, Указ № 809) — НЕ из сырого пользовательского
 * текста → PII-инвариант shared_scenarios не нарушается.
 *
 * Usage:
 *   pnpm exec tsx scripts/backfill-shared-format.ts                 # dry-run
 *   pnpm exec tsx scripts/backfill-shared-format.ts --apply
 *   pnpm exec tsx scripts/backfill-shared-format.ts --force
 */
...
async function main() {
  ...
  const rows = await db.select().from(sharedScenarios)
  console.log(`shared_scenarios in scope: ${rows.length}`)
  ...
  for (const row of rows) {
    processed++
    const original = row.anonymizedContent as Record<string, unknown> | null
    ...
    // 5. videoSearchQuery + санитизация video-text
